...
class Product extends Model
{
    ...
    {
        return $this->variations()->where('price','!=',null)->orderBy('price')->first() ?? 0;
    }
    public function getSaleCheckAttribute()
    {
        return $this->variations()->where('sale_price','!=',null)->where('date_on_sale_from','<',Carbon::now())->where('date_on_sale_to','>',Carbon::now())->orderBy('sale_price')->first() ?? 0;
    }
